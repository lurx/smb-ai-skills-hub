...
Data to collect: account export, Change History, Search Terms Report. **Validate**: data must cover ≥30 days and include the Search Terms Report before proceeding.
Platform references: `ads/references/google-audit.md` (full 80-check audit).
...
Render the Google Ads Health Score using the shared bar format in `ads/references/audit-process.md` with categories: Conversion Tracking (25%), Wasted Spend (20%), Account Structure (15%), Keywords (15%), Ads (15%), Settings (10%).

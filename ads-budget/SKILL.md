...
Data to collect: budget and performance data across all active platforms. References: `ads/references/budget-allocation.md` (allocation framework), `ads/references/bidding-strategies.md` (strategy decision trees).
Budget-specific validations: spend data must cover ≥14 days before kill/scale decisions; kill list candidates need ≥20 clicks or ≥$100 spend before recommending pause. Output includes a kill list and a scale list.
...
Render the Budget Allocation Health score using the shared bar format in `ads/references/audit-process.md` with categories: Allocation Strategy, Bidding Strategies, Scaling Readiness, Budget Sufficiency.

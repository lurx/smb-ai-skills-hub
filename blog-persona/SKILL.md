...
description: >
  ...
  contraction frequency, and summary box label. Apply the active persona when
  writing or editing posts to enforce consistent voice. Use when user says
  "persona", "voice", "tone", "writing style", "brand voice", "create persona",
  "use persona", "פרסונת כתיבה", "טון כתיבה", "קול המותג".
user-invokable: true
...
framework and CMI Brand Voice Chart. Personas ensure consistent voice whenever
blog content is written or edited.

If the project has a `VOICE.md` file (a learned voice profile, e.g. produced by
the blog-style skill), offer to seed the persona's style and tone values from it
...
| use `<name>` | Set active persona for current session |
| show `<name>` | Display full persona profile |
...
## Applying a Persona When Writing or Editing

When a persona is active, load the persona JSON and enforce these constraints
while drafting or revising posts:

1. **Pre-generation** - Load the persona and keep its tone dimensions and style
   rules in mind before drafting.
2. **During generation** - Follow do/dont rules, target sentence length
   mean/std, use contractions at the specified frequency.
...
`skills/blog-persona/references/active-persona.json` so the choice survives beyond
conversation-local state, and re-read it whenever writing or editing posts.

...
description: >
  ...
  "visualize data", "גרף לפוסט", "תרשים", "ויזואליזציה".
user-invokable: true
license: MIT
---
...
Generates dark-mode-compatible inline SVG charts for blog posts. Use it
whenever chart-worthy data appears in a post being written or edited, or when
the user asks for a chart directly.

For supported chart types, prefer the deterministic CLI bundled with this
skill:

```bash
python3 scripts/generate_chart_svg.py --input chart.json --output chart.html --json
```

## Input Format

Describe the chart as a request like this (gathered from the user or the
post's data):
